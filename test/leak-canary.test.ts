...
describe.skipIf(!live)('leak canary — a row is reachable only via workspace AND acl', () => {
  ...
  describe('cross-tenant: the registry sweep', () => {
    ...
    const ALLOWLIST: Record<string, string> = {
      ...
      delete_page:
        'mutating and DESTRUCTIVE: a cross-tenant positive control would have to delete a real page mid-sweep, and the sweep runs against the same fixtures every later case reads. Covered by test/lifecycle.test.ts, which proves a page in the other tenant is not addressable by id from here. Its pageIds batch arm inherits the same property by the same mechanism — the `id = any(...)` select runs inside withScopedTx, so RLS filters another tenant\'s ids out before any delete is issued, and they come back reported as "no such page, or you cannot read it" exactly as an invisible page must',
      rescope_pages:
        'mutating: a positive control would have to flip a real page between private and workspace mid-sweep, and every later case in this file reads those same fixtures. Its tenancy is inherited rather than independent: the page select runs inside withScopedTx exactly as delete_page does, and the acl it writes comes from aclForScope(ctx), never from params, so a caller cannot name another tenant workspace even in principle. Covered by test/lifecycle-batch.test.ts, which carries the positive control this sweep cannot: it asserts an admin may NOT flip a colleague page to private (aclForScope derives the private grant from the CALLER, so doing so would stamp the admin own grant and orphan the author), and that a slug collision at the target scope is reported without aborting the rest of the batch',
      replace_page:
